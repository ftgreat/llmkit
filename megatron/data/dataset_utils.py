...
def build_train_valid_test_datasets_with_prefixes(data_impl,
    ...
    print("Separate data paths provided for train, valid & test.")
...
def _build_train_valid_test_datasets(data_prefix, data_impl, splits_string,
    ...
    def print_split_stats(name, index):
        ...
        print('     sentence indices in [{}, {}) total of {} '
                     'sentences'.format(start_index, end_index,
                                        end_index - start_index), flush=True)
...
def get_indexed_dataset_(data_prefix, data_impl, dataset_type, skip_warmup):
    ...
    print(' > indexed dataset stats:', flush=True)
    print('    number of documents: {}'.format(
        indexed_dataset.doc_idx.shape[0] - 1), flush=True)
...
def get_samples_mapping(indexed_dataset,
    ...
    if torch.distributed.get_rank() == 0 and \
       not os.path.isfile(indexmap_filename):
        print(' > WARNING: could not find index map file {}, building '
              'the indices on rank 0 ...'.format(indexmap_filename), flush=True)
        ...
        start_time = time.time()
        print(' > building samples index mapping for {} ...'.format(
            name), flush=True)
        ...
        print(' > done building samples index maping', flush=True)
        np.save(indexmap_filename, samples_mapping, allow_pickle=True)
        print(' > saved the index mapping in {}'.format(
            indexmap_filename), flush=True)
        # Make sure all the ranks have built the mapping
        print(' > elasped time to build and save samples mapping '
                     '(seconds): {:4f}'.format(
    ...
    print(' > loading indexed mapping from {}'.format(
        indexmap_filename), flush=True)
    start_time = time.time()
    ...
    print('    total number of samples: {}'.format(
        samples_mapping.shape[0]), flush=True)
