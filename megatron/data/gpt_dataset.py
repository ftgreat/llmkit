...
def build_train_valid_test_datasets(data_prefix, data_impl, splits_string,
    ...
    if data_prefix:
        print("Single data path provided for train, valid & test", flush=True)
        ...
        print("Separate data paths provided for train, valid & test. Split string will be ignored.", flush=True)
    ...
    def print_split_stats(name, index):
        ...
        print('     document indices in [{}, {}) total of {} '
                     'documents'.format(splits[index], splits[index + 1],
                                        splits[index + 1] - splits[index]), flush=True)
...
def _build_dataset(dataset_name, data_prefix, data_impl, splits_string,
    ...
    print('    {}:'.format(dataset_name), flush=True)
    print('     document indices in [0, {}) total of {} '
                 'documents'.format(total_num_of_documents, total_num_of_documents), flush=True)
...
def get_indexed_dataset_(data_prefix, data_impl, skip_warmup):
    ...
    print('    number of documents: {}'.format(
        indexed_dataset.sizes.shape[0]), flush=True)
...
def _build_index_mappings(name, data_prefix, documents, sizes,
    ...
    # Load mappings.
    start_time = time.time()
    print(f" > loading doc-idx mapping from {idx_path['doc']}", flush=True)
    doc_idx = np.load(idx_path['doc'], allow_pickle=True, mmap_mode='r')

    print(f" > loading sample-idx mapping from {idx_path['sample']}", flush=True)
    sample_idx = np.load(idx_path['sample'], allow_pickle=True, mmap_mode='r')

    print(f" > loading shuffle-idx mapping from {idx_path['shuffle']}", flush=True)
    ...
    print('    total number of samples: {}'.format(
        sample_idx.shape[0]), flush=True)
    print('    total number of epochs: {}'.format(num_epochs), flush=True)
